...
from src.models.evaluation import optimize_threshold, oof_027, oof_033, oof_tabpfn  # noqa: E402
from src.models.inference import load_model, preprocess_test, save_metadata, save_submission  # noqa: E402
...
def main() -> None:
    ...
    x_native, y = preprocess_native_train(df_raw)
    ...
    print(
        f"  Meta OOF acc={accuracy_score(y, (meta_proba_oof >= 0.5).astype(int)):.4f} | "
        f"acc(thr={threshold:.4f})={meta_acc:.4f} | ROC-AUC={roc:.4f}"
    )
    print(
        f"  Coefs LR: 027={meta.coef_[0][0]:.3f}  033={meta.coef_[0][1]:.3f}  047={meta.coef_[0][2]:.3f}"
    )
    ...
    meta_proba_test = meta.predict_proba(
        np.column_stack([p027_test, p033_test, p047_test])
    )[:, 1]
    ...
    print(
        f"     Meta OOF acc(thr)={meta_acc:.4f} | ROC-AUC={roc:.4f} | umbral={threshold:.4f}"
    )
    print(
