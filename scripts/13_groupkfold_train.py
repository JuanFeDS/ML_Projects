...
from src.models.artifact_store import save_metadata, save_submission
from src.models.catboost_native import make_pool, oof_proba_groupkfold
from src.reports.experiments.log import get_next_exp_id
from src.scripts.common import CAT_FEATURES_NATIVE, preprocess_native_with_groups
...
def main() -> None:
    ...
    x, y, groups = preprocess_native_with_groups(df_raw)
    ...
    x_test, _ = preprocess_native_with_groups(df_test, is_test=True)

    predictions = (final_model.predict_proba(make_pool(x_test))[:, 1] >= 0.5).astype(bool)
    sub_path = save_submission(predictions, test_ids, exp_id)
    save_metadata(
        {"exp_id": exp_id, "model": "CatBoost_GroupKFold",
         "cv_strategy": "GroupKFold(n_splits=5, group=LastName)",
         "threshold": 0.5, "oof_acc": round(oof_acc, 4), "oof_roc_auc": round(oof_roc, 4),
         "cat_features": CAT_FEATURES_NATIVE, "n_train_samples": len(y),
         "n_groups": n_groups, "catboost_params": CATBOOST_PARAMS},
        exp_id, "groupkfold",
    )
