...
def _val_probas(tags_027: list, tabpfn_tag: str, tabpfn_estimators: int,
                scaler, target_encoder, df_train: pd.DataFrame) -> tuple:
    ...
    for tag in tags_027:
        ...
        x_aligned = x_val_scaled.reindex(columns=feat_cols, fill_value=0) if feat_cols else x_val_scaled
        probas.append(m.predict_proba(x_aligned)[:, 1])

    x_native, y_native = preprocess_native(df_train)
    ...
    probas.append(model_033.predict_proba(Pool(x_val_native, cat_features=CAT_FEATURES_NATIVE))[:, 1])

    print(f"  TabPFN val: fit en {len(x_native) - len(x_val_native):,}, predict en {len(x_val_native):,}...")
...
def main() -> None:
    ...
    parser.add_argument("--weights", nargs=3, type=float, default=[1.0, 1.0, 1.0],
    ...
    probas_val, y_val = _val_probas(
        [EXP_027_TAG], args.tabpfn_tag, args.tabpfn_estimators,
        scaler, target_encoder, df_train,
    )
    threshold, val_acc = optimize_threshold(y_val, sum(w * p for w, p in zip(w_arr, probas_val)))
    ...
    x_test_027 = preprocess_test(df_test, FEATURE_SETS[FS_017], get_feature_cols(model_027), scaler, target_encoder)
    ...
    save_metadata(
        {"exp_id": exp_id, "type": "ensemble_soft_voting_tabpfn", "models": all_tags,
         "weights": dict(zip(all_tags, w_arr.tolist())),
         "threshold": threshold, "val_accuracy": val_acc},
        exp_id, "ensemble_tabpfn",
    )
