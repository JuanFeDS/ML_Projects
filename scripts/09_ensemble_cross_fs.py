...
from src.preprocessing.common import CAT_FEATURES_NATIVE, FS_NATIVE, preprocess_native_test, preprocess_native_train
...
def _val_proba_fs017(tags: list, scaler, target_encoder) -> tuple:
    ...
    for tag in tags:
        ...
        x_aligned = (
            x_val.reindex(columns=feat_cols, fill_value=0) if feat_cols else x_val
        )
...
def _val_proba_033(df_train: pd.DataFrame) -> np.ndarray:
    ...
    x, y = preprocess_native_train(df_train)
    _, x_val, _, y_val = train_test_split(
...
def main() -> None:
    ...
    x_tests = [
        preprocess_test(
            df_test, FEATURE_SETS[FS_017], get_feature_cols(m), scaler, target_encoder
        )
    ...
    probas_test.append(
        model_033.predict_proba(Pool(x_test_native, cat_features=CAT_FEATURES_NATIVE))[
            :, 1
        ]
    )
    predictions = (sum(w * p for w, p in zip(w_arr, probas_test)) >= threshold).astype(
        bool
    )
    ...
    save_metadata(
        {
            "exp_id": exp_id,
            "type": "ensemble_soft_voting_cross_fs",
            "models": all_tags,
