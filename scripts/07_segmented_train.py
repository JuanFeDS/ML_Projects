...
def train_segment(df_seg, fs, cv, n_iter, seg_name, exp_id):
    ...
    print(f"  OOF acc={oof_acc:.4f} | ROC-AUC={oof_roc:.4f} | thr={threshold:.4f} -> acc={thr_acc:.4f}")
...
def main() -> None:
    ...
    print(f"Segmento active: {len(df_active):,} filas ({100*len(df_active)/len(df_raw):.1f}%)")
    ...
    for seg, a in arts.items():
        print(f"  {seg}: OOF={a['oof_acc']:.4f} | ROC={a['oof_roc']:.4f} (n={a['n_samples']:,})")
    print("=" * 60)
