...
Ejemplos:
    ...
    python run.py --from-train
    python run.py --predict-only
    python run.py --init                                   # inicializa MLflow
"""
import argparse
import sys
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Spaceship Titanic — pipeline ML (entrada unificada)"
    )
    parser.add_argument(
        "--stage",
        choices=("all", "pipeline", "eda", "features", "train", "predict"),
        default=None,
    ...
    parser.add_argument("--init", action="store_true", help="Inicializa MLflow y sale")
    ...
    if args.init:
        ...
        print("  UI           : mlflow ui --backend-store-uri", MLFLOW_TRACKING_URI)
        return
