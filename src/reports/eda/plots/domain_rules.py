...
def violations_table(violations_df: pd.DataFrame) -> go.Figure:
    ...
    row_colors = [
        "#ffd6d6" if v > 0 else "#d4f7d4" for v in violations_df["Violaciones"]
    ]
    return go.Figure(
        data=[
            go.Table(
                ...
                cells={
                    "values": [
                        violations_df[c].tolist() for c in violations_df.columns
                    ],
                    "fill_color": [row_colors]
                    + [["#f8f9fa"] * len(violations_df)]
                    * (len(violations_df.columns) - 1),
                    "align": "left",
                },
            )
        ]
...
def imputation_opportunities_bar(opps_df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Bar(
            x=opps_df["Variable"],
            y=opps_df["Resolubles"],
            name="Resolubles por inferencia",
            marker_color="#2ecc71",
    ...
    fig.add_trace(
        go.Bar(
            x=opps_df["Variable"],
            y=opps_df["No resolubles"],
            name="Requieren imputación estadística",
            marker_color="#e74c3c",
        )
    )
    fig.update_layout(
