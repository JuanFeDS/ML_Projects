...
def total_spending_compare(
    ...
    Args:
        raw: Tupla (group_false, group_true) con TotalSpending crudo.
        log_transformed: Tupla (group_false, group_true) con TotalSpending log.
        r_raw: Correlacion Pearson del spending crudo con target.
        r_log: Correlacion Pearson del spending log con target.
    """
    group_f_raw, group_t_raw = raw
    group_f_log, group_t_log = log_transformed
    fig = make_subplots(
        rows=1, cols=2, subplot_titles=["TotalSpending (crudo)", "TotalSpending_Log"]
    )
...
def spending_per_service_bar(per_service_df: pd.DataFrame) -> go.Figure:
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=[
            "Correlación Pearson (log1p) con target",
            "Transported rate: gastadores vs no",
        ],
    )
    fig.add_trace(
        go.Bar(
