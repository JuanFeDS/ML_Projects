...
def age_scale_compare(
    age_pre: pd.Series,
    age_post: pd.Series,
    mean_age: float,
    std_age: float,
) -> go.Figure:
    """Histograma de Age antes y despues del escalado estandar.

    Args:
        age_pre: Serie de Age original (sin escalar).
        age_post: Serie de Age escalada con StandardScaler.
        mean_age: Media de Age usada por el scaler.
        std_age: Desviacion estandar de Age usada por el scaler.
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=[
            "Age — antes de escalar",
            "Age — despues de StandardScaler",
        ],
    )
    fig.add_trace(
        go.Histogram(
    ...
    fig.update_layout(
        title_text=(
            f"Age — StandardScaler (media={mean_age:.2f}, std={std_age:.2f})"
        ),
        height=400,
    )
    return fig
