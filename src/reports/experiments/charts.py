...
def plot_leaderboard(df: pd.DataFrame) -> go.Figure:
    ...
    df_sorted["nota"] = df_sorted["anomaly"].apply(lambda a: "⚠ Data leakage" if a else "✓")

    row_base = ["#ffffff" if i % 2 == 0 else "#f8fafc" for i in range(len(df_sorted))]

    normal_mask = ~df_sorted["anomaly"]
    normal_colors = _color_scale(
        df_sorted.loc[normal_mask, "val_accuracy"], low="#fee2e2", high="#bbf7d0",
    )
    val_colors: List[str] = []
    ...
    normal_colors_light = _color_scale(
        df_sorted.loc[normal_mask, "val_accuracy"], low="#fff1f1", high="#f0fdf4",
    )
    fs_colors: List[str] = []
    ...
    cols_display = {
        "rank_label": "  #", "feature_set": "Feature Set", "val_accuracy": "Val Acc",
        "val_roc_auc": "Val AUC", "cv_accuracy": "CV Acc", "n_trials": "Trials",
        "best_trial_acc": "Best Trial", "git_commit": "Commit", "start_time": "Fecha",
        "nota": "Estado",
    }
    ...
    fig = go.Figure(data=[go.Table(
        ...
        cells=dict(
            ...
            font=dict(color=font_colors_per_col, size=12, family="Inter, monospace"),
