...
def load_experiment_data(
    tracking_uri: str, experiment_name: str = "spacechip_titanic"
) -> Dict[str, Any]:
    ...
    if exp is None:
        raise ValueError(
            f"Experimento '{experiment_name}' no encontrado en {tracking_uri}"
        )
    ...
    parent_map = {
        r.info.run_id: r.data.tags.get("mlflow.parentRunId", "") for r in all_runs
    }
    ...
    for run in all_runs:
        ...
        feature_set = (
            tags.get("feature_set") or _parse_fs_from_run_name(name) or "unknown"
        )
        ...
        training_rows.append(
            {
                ...
                or metrics.get("cv_accuracy_mean", None),
    ...
    best_df = (
        training_df.loc[best_idx].copy().sort_values("fs_num").reset_index(drop=True)
    )

    if not trials_df.empty:
        group_col = (
            "root_training_id"
            if "root_training_id" in trials_df.columns
            else "parent_run_id"
        )
        trial_counts = (
            trials_df.groupby(group_col)
            .agg(
            ...
            .reset_index()
            .rename(columns={group_col: "run_id"})
        )
    ...
    summary = {
        ...
        "best_feature_set": best_df.loc[~best_df["anomaly"], "feature_set"].iloc[
            best_df.loc[~best_df["anomaly"], "val_accuracy"].argmax()
        ],
